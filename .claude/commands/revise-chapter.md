...
1. Parse Buch- und Kapitelnummer aus `$ARGUMENTS`. Akzeptiere Formate wie "buch 2 kapitel 5", "b2 k5", "b2 k05". Falls nicht parsebar: frage den User mit AskUserQuestion.
2. Setze Variablen:
   - `BUCH` = Buchnummer
...
## Schritt 3 — Textliche Änderungen einarbeiten (Subagent pro Szene)

Für jede Szene, die textliche Verbesserungsvorschläge hat, rufe das **Agent-Tool** auf. Die Szenen können **parallel** bearbeitet werden:

```
Agent({
  description: "Textliche Überarbeitung K{KAP}-S{nn}",
  prompt: "Du bist ein Überarbeiter für ein Roman-Projekt.
...
2. Frage mit **AskUserQuestion**: "Soll ich diesen Vorschlag einarbeiten? (ja/nein/anpassen)"
3. Falls **ja**: Arbeite den Vorschlag per Agent-Tool ein (gleiche Regeln wie Schritt 3, aber mit dem inhaltlichen Vorschlag).
